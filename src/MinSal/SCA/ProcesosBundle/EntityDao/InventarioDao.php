...
class InventarioDao {
    ...
    function __construct($doctrine) {
        ...
        $this->repositorio = $this->doctrine->getRepository('MinSalSCAProcesosBundle:Inventario');
    }
    ...
    public function getInventarios($entId) {
        $registros = $this->em->createQuery("SELECT E
                                          FROM MinSalSCAProcesosBundle:Inventario E JOIN E.entidad A
                                          WHERE A.entId = :entId
    ...
    public function findInventario($entId, $alcId, $invGrado, $invNombreEsp) {
        $registros = $this->em->createQuery("SELECT E
                                          FROM MinSalSCAProcesosBundle:Inventario E JOIN E.entidad A JOIN E.alcohol B
                                          WHERE A.entId = :entId
                                            AND B.alcId = :alcId
    ...
    public function existeInventario($entId) {
        $result = $this->em->createQuery("SELECT count(e) 
                                          FROM MinSalSCAProcesosBundle:Inventarioe
                                          WHERE e.entId = :entId")
                 ->setParameter('entId',$entId);
